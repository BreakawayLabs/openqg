...
Layer variables are indexed with subscript $k$ from 1-$N$, increasing away from the ocean-atmosphere interface.
Layer interface variables are indexed with subscripts $k\uparrow$ and $k\downarrow$ for $k$ from 1-$N$. A subscript of $k\uparrow$ is used for the upper boundary of layer $k$ whilst $k\downarrow$ represents the lower boundary.
For a layer interface variable $X$ we introduce the function
\begin{equation}
\delta(X_k) = X_{k\uparrow} - X_{k\downarrow}.
\end{equation}

Note that it is assumed that $p_k$, $u_k$ and $v_k$ are effectively layer-averaged quantities, so that integration between $H_{k\downarrow}$ and $H_{k\uparrow}$ simply gives
\begin{equation}\label{eq:momq}
\left(q^*_k\right)_t  + \left(u_k q^*_k \right)_x  + \left(v_kq^*_k\right)_y  = \frac{f_0 \delta(w^*_k)}{\delta\left(H_{k}\right)} - A_4\nabla_H^4\zeta_k,
\end{equation}
which is an equation for the evolution of potential vorticity in the layer.
Evaluation of vertical velocity at the layer boundaries ($w_{k\uparrow}^*$ and $w_{k\downarrow}^*$) requires knowledge of the flux between layers (which we call $e$ for net entrainment velocity through the boundary of the layer).
Vertical velocity is a first order quantity and thus incorporates variable layer interface height $h$, giving
\begin{equation}\label{eq:w1}
w_{k\uparrow\downarrow}^* =  (h_{k\uparrow\downarrow})_t + (u_{k} h_{k\uparrow\downarrow})_x + (v_{k} h_{k\uparrow\downarrow})_y + e_{k\uparrow\downarrow}.
\end{equation}
This leads to
...
\begin{equation}
\left(q^*_k - \frac{f_0\delta\left(h_k\right)}{\delta(H_k)}\right)_t  + \left(u_k \left(q^*_k - \frac{f_0\delta\left(h_k\right)}{\delta(H_k)}\right)\right)_x  + \left(v_k\left(q^*_k -\frac{f_0\delta\left(h_k\right)}{\delta(H_k)}\right)\right)_y  = \frac{f_0 \delta\left(e_k\right)}{\delta\left(H_{k}\right)} - A_4\nabla_H^4\zeta_k.
\end{equation}
...
\begin{eqnarray}
\eta_{k\uparrow}   & \equiv & h_{k\uparrow}   - H_{k\uparrow} = \frac{\delta(p_{k\uparrow})}{\gp{}{k\uparrow}}\label{eq:eta1}\\
\eta_{k\downarrow} & \equiv & h_{k\downarrow} - H_{k\downarrow} = \frac{\delta(p_{k\downarrow})}{\gp{}{k\downarrow}}\label{eq:eta2}
\end{eqnarray}
Here we have introduced $\gp{}{k\uparrow\downarrow}$, the reduced gravity across the $k\uparrow\downarrow$ interface, given by
\begin{eqnarray}
\gp{}{k\uparrow}   & = & \frac{g\delta(\rho_{k\uparrow})}{\rhb{}_0} \\
\gp{}{k\downarrow} & = & \frac{g\delta(\rho_{k\downarrow})}{\rhb{}_0}.
\end{eqnarray}
Care must be taken when evaluating the $\delta()$ function to find the difference between two layer variables. In the ocean case we have
\begin{eqnarray}
\delta(X_{k\uparrow}) & = & X_{k-1} - X_k\\
\delta(X_{k\downarrow}) & = & X_k - X_{k+1}
\end{eqnarray}
whereas in the atmosphere we have
\begin{eqnarray}
\delta(X_{k\uparrow}) & = & X_{k+1} - X_k\\
\delta(X_{k\downarrow}) & = & X_k - X_{k-1}.
\end{eqnarray}
...
\begin{eqnarray}
\textrm{ocean: } \quad \eta_{N\downarrow} & = & D - H_N \\
\textrm{atmos: } \quad \eta_{N\uparrow} & = & 0,
\end{eqnarray}
...
\begin{eqnarray}
\textrm{ocean: } \quad \eta_{1\uparrow} & = & 0 \\
\textrm{atmos: } \quad \eta_{1\downarrow} & = & T,
\end{eqnarray}
where the topography $T$ is zero over the ocean and is assumed to be small compared with the unperturbed layer thickness $H_1$, for consistency with the quasigeostrophic scaling.

Combining (\ref{eq:eta1}) and (\ref{eq:eta2}) lets us write
\begin{equation}
\delta(h_k) = \delta(\eta_k),
\end{equation}
where $\delta(\eta_k) = \eta_{k\uparrow} - \eta_{k\downarrow}$ is a linear function of pressure.
...
\mtx{A} =
\left[ \begin{array}{cccccc}
    \frac{1}{\delta(H_1) \gp{}{1\uparrow}} & \frac{-1}{\delta(H_1) \gp{}{1\uparrow}} &&\\
