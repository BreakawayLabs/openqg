function qgnormeofs(base_dir,run,printflag,frecut)
% QGNORMEOFS  Find and plot filtered EOFS from OpenQG run
%   QGNORMEOFS(BASE_DIR,RUN,PRINTFLAG,FRECUT) takes filtered data
...
%   VERSION LOG
%   v1.0 - created from qgfilteofs_v1.4.m by AH, 16/6/04
%   v1.1 - updated for Q-GCM V1.3 - AH 27/8/04 
%        - now generalised with new qgfilteofs incl. all aspect ratios
%   v1.2 - updated for Q-GCM V1.4 - AH 13/4/07 
%   

tic
disp('CALCULATING EOFS OF NORMALISED DATA SET:')
disp('----------------------------------------')

% Define incoming and outgoing filenames:
outfile=[base_dir,run,'/','normeofs.mat'];
...
if ~(oceanonly)
  ...
  if outflat(2)==1
    ...
    str1 = [run,': Normalised atmosphere pressure EOFS and PCs'];
    eofplot(xa,ya,ta,pa1vv,pa1Dext,pa1Dperc,pa1pcs,xpa,ypa,pa1bar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normatpeofs.pdf'])
    end
    ...
    str1 = [run,': Normalised atmosphere height EOFS and PCs'];
    eofplot(xa,ya,ta,ha1vv,ha1Dext,ha1Dperc,ha1pcs,xpa,ypa,ha1bar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normatheofs.pdf'])
    end
  ...
  if outflat(1)==1
    ...
    str1 = [run,': Normalised AST EOFS and PCs'];
    eofplot(xa,ya,ta,astvv,astDext,astDperc,astpcs,xta,yta,astbar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normatteofs.pdf'])
    end
...
if ~(atmosonly)
  ...
  if outfloc(2)==1
    ...
    str1 = [run,': Normalised ocean pressure EOFS and PCs'];
    eofplot(xo,yo,to,po1vv,po1Dext,po1Dperc,po1pcs,xpo,ypo,po1bar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normocpeofs.pdf'])
    end
    ...
    str1 = [run,': Normalised ocean height EOFS and PCs'];
    eofplot(xo,yo,to,ho1vv,ho1Dext,ho1Dperc,ho1pcs,xpo,ypo,ho1bar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normocheofs.pdf'])
    end
  ...
  if outfloc(1)==1
    ...
    str1 = [run,': Normalised SST EOFS and PCs'];
    eofplot(xo,yo,to,sstvv,sstDext,sstDperc,sstpcs,xto,yto,sstbar,str1,frecut)
    if printflag
      print('-dpdf',[base_dir,run,'/','normocteofs.pdf'])
    end
